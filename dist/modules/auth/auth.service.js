...
let AuthService = class AuthService {
    ...
    jwtService;
    mailerService;
    constructor(usersRepo, jwtService, mailerService) {
    ...
    async register(dto) {
        ...
        if (dto.phoneNumber)
            conditions.push({ phoneNumber: dto.phoneNumber });
        const existing = await this.usersRepo.findOne({ where: conditions });
        if (existing)
            throw new common_1.ConflictException('Email hoặc số điện thoại đã tồn tại');
    ...
    async validateUser(account, pass) {
        if (!account || !pass)
