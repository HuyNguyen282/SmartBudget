...
function Reveal({ children, delay = "" }) {
    ...
    return (<div ref={ref} className={`reveal ${delay} ${visible ? "in" : ""}`.trim()}>
      {children}
    </div>);
}
function Navbar() {
    return (<nav className="nav">
      <image_1.default src="/download.svg" alt="Logo" className="nav-logo" width={130} height={100}/>
      ...
      <div className="app-window">
        ...
        <div className="app-body">
          <div className="sidebar">
            {[["⊞", "Tổng quan"], ["↗", "Giao dịch"], ["◎", "Ngân sách"], ["↘", "Báo cáo"]].map(([icon, label], i) => (<div key={label} className={`s-item${i === 0 ? " active" : ""}`}><span>{icon}</span>{label}</div>))}
          </div>
          <div className="main">
...
function Features() {
    ...
    return (<section className="section" id="tinh-nang" style={{ background: "#fff", textAlign: "center" }}>
      ...
      <div className="feat-grid">
        {feats.map((f, i) => (<Reveal key={f.title} delay={`d${i + 1}`}>
...
function Report() {
    ...
    return (<section className="section report-sec" id="bao-cao">
        ...
        <Reveal>
          <div>
            ...
            <ul className="check-list">
              {["Phân tích thu chi chi tiết theo từng danh mục", "Dự báo chi tiêu tháng tới dựa trên lịch sử", "Xuất báo cáo PDF & Excel chuyên nghiệp"].map(c => (<li key={c}><div className="chk">✓</div>{c}</li>))}
            </ul>
          </div>
        </Reveal>
        <Reveal delay="d1">
          <div className="spend-card">
            ...
            {spends.map(s => (<div className="spend-row" key={s.label}>
...
function Pricing() {
    ...
    return (<section className="section" id=" bang-gia" style={{ background: "#fff", textAlign: "center" }}>
...
function Support() {
    ...
    return (<section className="section support-sec" id="ho-tro">
      <Reveal>
      ...
      <div className="support-grid">
        {channels.map((c, i) => (<Reveal key={c.title} delay={`d${i + 1}`}>
...
function CTA() {
    return (<section className="cta-sec">
      <Reveal>
