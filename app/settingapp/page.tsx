...
import { getSettings, saveSettings, BASE_URL } from "@/lib/api";
...
const EMPTY_PROFILE: UserProfile = {
  ...
  currency: "VNĐ - Đồng Việt Nam", language: "Tiếng Việt",
};
...
export default function SettingsPage() {
    ...
    async function load() {
      try {
        ...
        setProfile((p) => ({
          ...
          name: data.name,
          email: data.email ?? '',
          currency: data.currency ?? "VNĐ - Đồng Việt Nam",
        }));
      } catch { }
    }
    load();
  ...
  async function handleChangePassword() {
    ...
    if (pwForm.next !== pwForm.confirm)
      return setPwError("Mật khẩu mới không khớp.");
    if (pwForm.next.length < 8)
      return setPwError("Mật khẩu phải có ít nhất 8 ký tự.");
    try {
      ...
      setPwError(Array.isArray(msg) ? msg[0] : msg || "Mật khẩu hiện tại không đúng.");
    }
  }
              ...
              {tab === "notifications" && (
                <div>
                  ...
                  <div className="space-y-3">
                    {[
                      { icon: Mail, label: "Email thông báo", desc: "Nhận báo cáo hàng tuần và cảnh báo ngân sách qua email.", value: emailNotif, set: setEmailNotif },
                      { icon: Smartphone, label: "Thông báo trình duyệt", desc: "Hiển thị cảnh báo trực tiếp trên màn hình khi có giao dịch lớn.", value: browserNotif, set: setBrowserNotif },
                    ...
                    <div className="space-y-4 max-w-sm">
                      ...
                      {pwSaved && (
                        <p className="text-xs text-green-500 flex items-center gap-1">
                          <CheckCircle2 className="w-3.5 h-3.5" /> Đổi mật khẩu thành công!
                        </p>
                      )}
                      <button
