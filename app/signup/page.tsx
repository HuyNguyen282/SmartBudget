...
export default function SignUpPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Kiểm tra ở Frontend (giữ nguyên)
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/auth/register`, {
        ...
        body: JSON.stringify({
          ...
          // SỬA Ở ĐÂY: Key phải là confirmPassword (có chữ P viết hoa)
      ...
      if (!res.ok) {
        ...
        // Nếu server báo "Mật khẩu nhập lại không khớp", 
        // chính là do cái key confirmPassword ở trên bị sai hoặc thiếu
        throw new Error((d as any).message ?? "Đăng ký thất bại");
      }
  ...
  return (
    <div className="min-h-screen flex bg-[#f7f7f9] font-sans">
      ...
      {/* ── LEFT PANEL ── */}
      <div className="flex-1 flex flex-col justify-center px-12 xl:px-24 py-12 max-w-[580px]">
        ...
        {/* Heading */}
        <div className="mb-8">
          <p className="text-sm font-semibold text-blue-600 uppercase tracking-widest mb-3">Bắt đầu miễn phí</p>
          <h1 className="text-[2.2rem] font-extrabold text-gray-900 leading-[1.2] tracking-tight">
            Create Your Account!<br />
            <span className="text-gray-500 text-2xl font-semibold">I hope you have a great experience</span>
          </h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <input required placeholder="First Name" value={form.firstName}
              ...
              className="px-5 py-4 rounded-2xl border border-gray-200 bg-white text-sm text-gray-800 placeholder-gray-400 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-50 transition-all shadow-sm" />
          ...
          <div className="relative">
            <input type={showPass ? "text" : "password"} required placeholder="Enter Your Password" value={form.password}
          ...
          {/* Confirm */}
          <div className="relative">
            <input type={showConfirm ? "text" : "password"} required placeholder="Confirm Your Password" value={form.confirm}
          ...
          <button type="submit" disabled={loading}
            ...
            {loading ? (
              <><svg className="w-4 h-4 animate-spin" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4l3-3-3-3v4a8 8 0 00-8 8h4z" /></svg>Đang đăng ký...</>
            ) : "Sign up"}
          </button>
        </form>
        ...
        <p className="text-center text-sm text-gray-500 mt-5">
          ...
          <Link href="/signin" className="text-blue-600 font-bold hover:underline">Login</Link>
        </p>
      </div>
          ...
          <div className="bg-gray-900 rounded-[18px] p-2.5 shadow-2xl">
            <div className="bg-white rounded-[10px] overflow-hidden">
              ...
              <div className="bg-gray-50 px-4 py-3">
                ...
                {[
                  { icon: "🍜", name: "Ăn uống", bar: "bg-green-400", w: "w-full", total: "15,500,000đ" },
                  { icon: "🛍", name: "Mua sắm", bar: "bg-yellow-400", w: "w-1/6", total: "2,500,000đ" },
                  { icon: "📋", name: "Hóa đơn", bar: "bg-gray-300", w: "w-1/12", total: "300,000đ" },
                ].map((r) => (
      ...
      {/* Avatar */}
      <div className="fixed bottom-5 left-5 w-9 h-9 rounded-full bg-gray-900 flex items-center justify-center text-white text-sm font-bold shadow-lg">N</div>
    </div>
  );
}